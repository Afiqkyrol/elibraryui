...
import { updateNotApprovedUser } from "@/api/admin/putApi";
import AdminLeftSideBar from "@/app/elibrary_ui/admin/component/AdminLeftSideBar";
import { deleteUser } from "@/api/admin/deleteApi";
import { ToastContainer, toast } from "react-toastify";
import "react-toastify/dist/ReactToastify.css";
import { useRouter } from "next/navigation";
import { sendStatusUserEmail } from "@/api/email/emailApi";
...
export default function ShowEditUserPage({ params }) {
  ...
  useEffect(() => {
    async function getUser() {
      ...
      setApprovedBy(localStorage.getItem("username"));
    }
  ...
  async function deleteHandler() {
    try {
      ...
      router.push("/elibrary_ui/admin/user/new");
    } catch (error) {
      toast.error("Error");
    }
  }
  ...
  async function submitHandler(e) {
    ...
    try {
      ...
      if (approved === "yes") {
        await sendStatusUserEmail(params.userId);
      }
        ...
        <div>
          <center>
            ...
            >
              ...
              <label className="block mb-2">Approve</label>
              <select
                name="approve"
