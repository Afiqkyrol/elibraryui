...
export default function ApprovedUserTable(props) {
  return (
    <div className="container" style={{ width: "90%", overflowX: "scroll" }}>
      <div className="flex gap-4">
        <Link href="/elibrary_ui/admin/user/add">
          <button className="bg-green-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
            Add User
          </button>
        </Link>
        <Link href="/elibrary_ui/admin/user/new">
          <p className=" hover:underline py-2 px-4 ">New Registered User</p>
        </Link>
      </div>
      <br></br>
        ...
        <tbody className="bg-white divide-y divide-gray-200">
          {props.data.map((user, index) => (
            <tr key={index}>
              ...
              <td>
                <Link href={`/elibrary_ui/admin/user/${user.id}`}>
