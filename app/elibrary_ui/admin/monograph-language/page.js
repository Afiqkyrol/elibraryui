...
import { fetchMonoLanguageList } from "@/api/admin/getApi";
import MonoLangTable from "../component/MonoLangTable";
import { ToastContainer, toast } from "react-toastify";
import "react-toastify/dist/ReactToastify.css";

export default function ShowMonoLangList() {
  const [isLoading, setIsLoading] = useState(true);
  const [monographLangList, setMonographLangList] = useState([]);

  useEffect(() => {
    async function getMonographLang() {
      setMonographLangList(await fetchMonoLanguageList());
    ...
    <div className="flex h-screen">
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div>
          <center>
            <br></br>
            <MonoLangTable data={monographLangList} />
          </center>
        </div>
      </div>
    </div>
